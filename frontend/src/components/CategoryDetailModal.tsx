...
export const CategoryDetailModal: React.FC<CategoryDetailModalProps> = ({ category, hands, onClose, onUpdate }) => {
  ...
  return (
    <div style={{
      ...
      <div style={{ backgroundColor: 'white', padding: '20px', borderRadius: '8px', display: 'flex', flexDirection: 'column', alignItems: 'center', maxWidth: '90vw', maxHeight: '90vh', overflowY: 'auto' }}>
        <h3 style={{ marginTop: 0, marginBottom: '20px' }}>Détails - {category}</h3>
        <RangeSelector 
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px', marginTop: '20px', width: '100%' }}>
          ...
          <button onClick={handleSave} style={{ padding: '8px 15px', cursor: 'pointer', border: 'none', borderRadius: '4px', backgroundColor: '#3498db', color: 'white' }}>Enregistrer</button>
        </div>
      </div>
    </div>
  );
