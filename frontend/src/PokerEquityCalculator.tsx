...
const PokerEquityCalculator: React.FC = () => {
  ...
  const handleValidateStreet = () => {
    ...
    // Could show a toast or auto-collapse.
  };
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto', padding: '20px', fontFamily: 'sans-serif' }}>
      <h1 style={{ textAlign: 'center', color: '#2c3e50' }}>Calculateur d'Équité Poker</h1>
      
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px', backgroundColor: '#fff', padding: '20px', borderRadius: '8px', boxShadow: '0 4px 6px rgba(0,0,0,0.1)' }}>
        
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '20px' }}>
          <div style={{ flex: '1 1 300px', backgroundColor: '#f8f9fa', borderRadius: '8px', padding: '15px', border: '1px solid #e0e0e0' }}>
            <div 
              onClick={() => setIsPlayerHandOpen(!isPlayerHandOpen)}
              style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', cursor: 'pointer' }}
            >
              <label style={{ fontWeight: 'bold', margin: 0, cursor: 'pointer' }}>
        ...
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
            ...
            >
              Valider la sélection ({currentStreet})
            </button>
          </div>
          ...
          <div style={{ marginTop: '10px', fontSize: '12px', color: '#7f8c8d', maxHeight: '60px', overflowY: 'auto', wordBreak: 'break-all' }}>
      ...
      {error && (
        <div style={{ marginTop: '20px', padding: '15px', backgroundColor: '#e74c3c', color: 'white', borderRadius: '4px' }}>
      ...
      {result && (
        <div style={{ marginTop: '20px', padding: '20px', backgroundColor: '#fff', borderRadius: '8px', boxShadow: '0 4px 6px rgba(0,0,0,0.1)' }}>
          <h2 style={{ textAlign: 'center', margin: '0 0 15px 0', color: '#27ae60' }}>
            Équité : {(result.equity * 100).toFixed(2)}%
          </h2>
          
          <div style={{ height: '20px', backgroundColor: '#e0e0e0', borderRadius: '10px', overflow: 'hidden', marginBottom: '20px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '10px', textAlign: 'center' }}>
            <div style={{ padding: '10px', backgroundColor: '#f8f9fa', borderRadius: '4px' }}>
              <div style={{ fontSize: '12px', color: '#7f8c8d' }}>Victoires</div>
              <div style={{ fontWeight: 'bold' }}>{result.wins.toLocaleString()}</div>
            </div>
            <div style={{ padding: '10px', backgroundColor: '#f8f9fa', borderRadius: '4px' }}>
              <div style={{ fontSize: '12px', color: '#7f8c8d' }}>Égalités</div>
              <div style={{ fontWeight: 'bold' }}>{result.ties.toLocaleString()}</div>
            </div>
            <div style={{ padding: '10px', backgroundColor: '#f8f9fa', borderRadius: '4px' }}>
              <div style={{ fontSize: '12px', color: '#7f8c8d' }}>Défaites</div>
              <div style={{ fontWeight: 'bold' }}>{result.losses.toLocaleString()}</div>
            </div>
          </div>
